...
export const ReservationMeal = () => {
  ...
  const onSubmit = async (data: OnMealType) => {
    const res = await mutateAsync({ id: data.id, tableId: data.tableId, customerNum: Number(data.customerNum) });
    setIsOpen(false);
    ...
    <div className="flex flex-1 flex-col space-y-6">
      ...
      >
        <div className='space-y-4 text-black/85'>
          ...
          <TextField label="請輸入顧客姓名" type="text" {...register('name', { required: '此為必填' }) } placeholder='輸入姓名'/>

          <TextField label="請輸入電話" type="text" {...register('phone', { required: '此為必填' })} placeholder='輸入電話'/>
          ...
          <div className="flex flex-col space-y-2">
            <label>選擇桌次 :</label>
              <select className='rounded-md border border-black/10' {...register('tableId', { required: '此為必填' })}
                value={table} onChange={(e) => setTable(e.target.value)}>
                {
                  list.map((t) => (
                    <option value={t.id} key={t.id} disabled={t.status === TableStatus.MEAL}>
                      {`${t.name} ${t.status}`}
                    </option>
                  ))
                }
